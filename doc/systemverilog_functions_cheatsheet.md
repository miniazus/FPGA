...
*These functions generate physical hardware (Gates, Adders, Trees).*
...
*These run during compilation. If inputs are parameters/types, they cost **Zero Hardware**.*
...
### 3. Type Conversion
*Mostly used for reinterpreting wires. Usually zero cost.*
...
*Used in properties and assertions to check signal history. Synthesizable.*
...
### 5. Memory Loading
*Used to initialize RAMs/ROMs from files.*
...
### 6. Randomization (Simulation Only)
*Used in Testbenches. Never use in RTL.*
...
### 7. Display & I/O (Simulation Only)
*Console printing and file access.*
...
### 8. Simulation Control (Simulation Only)
*Time management and stopping the simulator.*
...
### 9. Advanced Math (Simulation Only)
*Complex math requiring C libraries. Needs IP Cores for hardware.*

| Function | Description | Synth? | Hardware Cost | Usage Example |
| :--- | :--- | :---: | :--- | :--- |
| **`$sqrt(x)`** | Square Root. | ❌ | **Very High** (Requires IP) | `y = $sqrt(x);` |
| **`$pow(x, y)`** | Power ($x^y$). | ❌ | **Very High** (Requires IP) | `y = $pow(2, x);` |
